...
__all__ = [
    ...
    'AlternatingSizeDecomposer', 'RandomSubproblemDecomposer', 'TilingChimeraDecomposer', 
...
class AlternatingSizeDecomposer(traits.ProblemDecomposer, traits.SISO, Runnable):
    """Selects a subproblem of variables maximally contributing to the problem
    energy.
    ...
    @classmethod
    def _same_col(cls, bqm, sample, ordered_priority, visited, size, num_rel, num_pred):
        rel_cutoff = num_rel * (num_rel - 2)
        pred_cutoff = (num_rel + num_pred) * (num_rel - 2)
        variables = []
        source = int()
        cnt = 0
        for v in ordered_priority:
            if v not in visited:
                source = list(ordered_priority.keys())[cnt]
                break
            cnt += 1
        
        if source < rel_cutoff:
            i = source % (num_rel - 2)

            while i < rel_cutoff:
                variables.append(i)
                i += num_rel - 2
        elif source < pred_cutoff:
            i = source % (num_rel - 2) + rel_cutoff

            while i < pred_cutoff:
                variables.append(i)
                i += num_rel - 2
        else:
            increments = (len(ordered_priority) - pred_cutoff) // (num_rel - 2)
            section = (source - pred_cutoff) // increments

            for i in range(increments):
                variables.append(pred_cutoff + (section * increments) + i)

        return variables
    
    def _same_col_both(cls, bqm, sample, ordered_priority, visited, size, num_rel, num_pred):
        rel_cutoff = num_rel * (num_rel - 2)
        pred_cutoff = (num_rel + num_pred) * (num_rel - 2)
        variables = []
        source = int()
    ...
    @classmethod
    def _same_col_extra(cls, bqm, sample, ordered_priority, visited, size, card, pred, pred_sel):
        num_rel = len(card)
        num_pred = len(pred)
        rel_cutoff = num_rel * (num_rel - 2)
        pred_cutoff = (num_rel + num_pred) * (num_rel - 2)
        ...
        if source < rel_cutoff:
            ...
            if i != 0 and source - 1 not in visited:
                variables.append(source - 1)
            if i != num_rel - 3 and source + 1 not in visited:
                variables.append(source + 1)
            ...
            i = source % (num_rel - 2) + rel_cutoff
            j = (source - rel_cutoff) // (num_rel - 2)

            if pred[j][0] * (num_rel - 2) + i not in visited:
                variables.append(pred[j][0] * (num_rel - 2) + (i - rel_cutoff))
            if pred[j][1] * (num_rel - 2) + i not in visited:
                variables.append(pred[j][1] * (num_rel - 2) + (i - rel_cutoff))
            ...
            for i in range(increments):
                if pred_cutoff + (section * increments) + i not in visited:
                    variables.append(pred_cutoff + (section * increments) + i)

        return variables
    
    @classmethod
    def _same_col_adjust(cls, bqm, sample, ordered_priority, visited, size, num_rel, num_pred):
        rel_cutoff = num_rel * (num_rel - 2)
        pred_cutoff = (num_rel + num_pred) * (num_rel - 2)
        variables = []
        source = int()
        ...
        if (num_pred + num_rel <= 10):
            ...
            if source < rel_cutoff:
                ...
                limit = 0
                while i < rel_cutoff and limit < size:
                    if i not in visited and i not in variables:
                        variables.append(i)
                        limit += 1
                    i += num_rel - 2
                ...
                limit = 0
                while i < pred_cutoff and limit < size:
                    if i not in visited and i not in variables:
                        variables.append(i)
                        limit += 1
                    i += num_rel - 2
    ...
    @classmethod
    def _same_col_limited(cls, bqm, sample, ordered_priority, visited, size, num_rel, num_pred):
        rel_cutoff = num_rel * (num_rel - 2)
        pred_cutoff = (num_rel + num_pred) * (num_rel - 2)
        variables = []
        source = int()
    ...
    @classmethod
    def _same_col_limited_energy(cls, bqm, sample, ordered_priority, visited, size, num_rel, num_pred):
        rel_cutoff = num_rel * (num_rel - 2)
        pred_cutoff = (num_rel + num_pred) * (num_rel - 2)
        variables = []
        source = int()
        ...
        if source < rel_cutoff:
            ...
            limit = 0
            marker = 0
            length = len(list(ordered_priority.keys()))
            while i < rel_cutoff and limit < size and marker < length:
                key = list(ordered_priority.keys())[marker]
                if key < rel_cutoff and key % (num_rel - 2) == i and key not in visited and key not in variables:
            ...
            marker = 0
            length = len(list(ordered_priority.keys()))
            while i < pred_cutoff and limit < size and marker < length:
                key = list(ordered_priority.keys())[marker]
                if pred_cutoff > key >= rel_cutoff and key % (num_rel - 2) == i and key not in visited and key not in variables:
                    variables.append(i)
                    limit += 1
                marker += 1
        else:
    ...
    @classmethod
    def _diff_col(cls, bqm, sample, ordered_priority, visited, size, card, pred, pred_sel):
        num_rel = len(card)
        num_pred = len(pred)
        rel_cutoff = num_rel * (num_rel - 2)
        pred_cutoff = (num_rel + num_pred) * (num_rel - 2)
        ...
        if source < rel_cutoff:
            i = source % (num_rel - 2) + rel_cutoff
            if i != rel_cutoff and source - 1 not in visited:
                variables.append(source - 1)
            if i != rel_cutoff + num_rel - 3 and source + 1 not in visited:
                variables.append(source + 1)
            ...
            if pred[j][0] * (num_rel - 2) + i not in visited:
                variables.append(pred[j][0] * (num_rel - 2) + i)
            if pred[j][1] * (num_rel - 2) + i not in visited:
                variables.append(pred[j][1] * (num_rel - 2) + i)
    ...
    def __init__(self, size, card, pred, pred_sel, min_gain=None,
                 rolling=True, rolling_history=1.0, silent_rewind=True,
                 traversal='energy', **runopts):
        ...
        traversers = {
            ...
            'bfs': partial(self._iterative_graph_search, method=self._bfs_nodes),
            'pfs': partial(self._iterative_graph_search, method=self._pfs_nodes),
            'same_col': partial(self._same_col, num_rel=len(card), num_pred=len(pred)),
            'same_col_adjust': partial(self._same_col_adjust, num_rel=len(card), num_pred=len(pred)),
            'same_col_extra': partial(self._same_col_extra, card=card, pred=pred, pred_sel=pred_sel),
            'same_col_limited': partial(self._same_col_limited, num_rel=len(card), num_pred=len(pred)),
            'same_col_limited_energy': partial(self._same_col_limited_energy, num_rel=len(card), num_pred=len(pred)),
            'same_col_both': partial(self._same_col_both, num_rel=len(card), num_pred=len(pred)),
            'diff_col': partial(self._diff_col, card=card, pred=pred, pred_sel=pred_sel),
        }

        super(AlternatingSizeDecomposer, self).__init__(**runopts)
